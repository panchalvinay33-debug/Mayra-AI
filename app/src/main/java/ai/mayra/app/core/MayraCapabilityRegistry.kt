...
object MayraCapabilityRegistry {
    val capabilities: List<MayraCapability> = listOf(
        ...
        MayraCapability("core.provider-eligibility", "Core assistant", "Provider and tool eligibility rules", MayraCapabilityStatus.DONE, "Runtime capability gates produce EXECUTE, CONFIRM, CLARIFY, FALLBACK or BLOCK plans without invoking providers."),
        MayraCapability("core.runtime-boundary", "Core assistant", "Audited typed runtime boundary", MayraCapabilityStatus.DONE, "Typed results, handler isolation, action idempotency, bounded persistent activity history and replay-safe confirmation tokens are covered by regressions."),
        MayraCapability("core.runtime-integration", "Core assistant", "Concrete assistant runtime integration", MayraCapabilityStatus.DEVICE_VERIFY, "Main-chat typed bridge, exact-action Confirm/Cancel UX, Current-only retrieval, Activity History and the narrow safe action are implemented; owner-device validation remains."),
        ...
        MayraCapability("memory.user-controlled", "Memory", "User-controlled personal memory", MayraCapabilityStatus.IN_PROGRESS, "Consent-first schema, explicit approval, sensitive-memory exclusions, provenance, expiry, edit/delete, deterministic retrieval and bounded Android persistence are implemented; user-facing controls and chat proposal UX remain."),
        ...
        MayraCapability("actions.typed-execution", "Actions", "Typed confirmed action execution", MayraCapabilityStatus.DEVICE_VERIFY, "Confirmation, duplicate prevention, persistent audit, chat UX and a narrow system-picker adapter are implemented; physical validation and broader reviewed adapters remain."),
