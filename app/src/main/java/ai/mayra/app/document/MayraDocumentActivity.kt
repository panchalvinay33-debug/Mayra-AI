...
private fun DocumentLibraryScreen(onClose: () -> Unit) {
    ...
    Scaffold { padding ->
        Column(
            ...
            Text("Choose documents you want Mayra to remember as local library entries. Mayra keeps only the URI and file metadata in this foundation build.")

            Button(
                onClick = { picker.launch(arrayOf("application/pdf", "text/*", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")) },
            ...
            Card(Modifier.fillMaxWidth()) {
                Column(Modifier.fillMaxWidth().padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                    ...
                    Text("Library, persistent access, search and open are implemented. Full text extraction, PDF page search and AI summaries are intentionally not claimed until parser and device testing are complete.", style = MaterialTheme.typography.bodySmall)
                }
            }
...
@Composable
private fun DocumentCard(document: MayraDocument, onOpen: () -> Unit, onRemove: () -> Unit) {
    Card(Modifier.fillMaxWidth()) {
