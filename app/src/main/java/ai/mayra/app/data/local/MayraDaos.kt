...
interface MemoryDao {
    ...
    @Query("SELECT * FROM memories ORDER BY importance DESC, updatedAt DESC, id DESC")
    fun observeAll(): Flow<List<MemoryEntity>>

    @Query("SELECT * FROM memories WHERE id = :id LIMIT 1")
    suspend fun getById(id: Long): MemoryEntity?

    @Query("SELECT * FROM memories WHERE category = :category ORDER BY importance DESC, updatedAt DESC, id DESC LIMIT :limit")
    suspend fun getByCategory(category: String, limit: Int = 50): List<MemoryEntity>

    @Query("SELECT * FROM memories WHERE content LIKE '%' || :query || '%' ORDER BY importance DESC, updatedAt DESC, id DESC LIMIT :limit")
...
interface ConversationDao {
    ...
    @Query("SELECT * FROM conversations WHERE sessionId = :sessionId ORDER BY createdAt ASC, id ASC")
    fun observeSession(sessionId: String): Flow<List<ConversationEntity>>

    @Query("SELECT * FROM conversations WHERE sessionId = :sessionId ORDER BY createdAt DESC, id DESC LIMIT :limit")
...
interface ReminderDao {
    ...
    @Query("SELECT * FROM reminders WHERE completed = 0 ORDER BY triggerAt ASC, id ASC")
    fun observePending(): Flow<List<ReminderEntity>>

    @Query("SELECT * FROM reminders WHERE completed = 0 AND triggerAt <= :now ORDER BY triggerAt ASC, id ASC")
...
interface GoalDao {
    ...
    @Query("SELECT * FROM goals WHERE status = :status ORDER BY priority DESC, updatedAt DESC, id DESC")
