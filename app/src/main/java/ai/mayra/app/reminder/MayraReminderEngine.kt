...
/** Deterministic English/Hindi/Hinglish reminder parser for common personal reminders. */
class MayraReminderParser(
    ...
    fun parse(raw: String): ReminderParseResult {
        ...
        return ReminderParseResult.NeedsClarification(
            "I understood the reminder, but not the time. Try ‘in 20 minutes’, ‘after 3 min’, ‘tomorrow at 8 PM’, or ‘kal subah 7 baje’."
        )
    }
    ...
    private fun parseClockTime(text: String): LocalTime? {
        ...
        if (match != null) {
            ...
            val hasTimeSignal = rawMatch.contains(':') || rawMatch.contains('.') ||
                ...
                text.containsAny(
                    "at $hourToken", "ko $hourToken", "subah", "सुबह", "shaam", "शाम",
                    "raat", "रात", "evening", "morning", "night", "afternoon"
                )
    ...
    private fun parsedWithTimeRemoved(
        original: String,
        pattern: Regex,
        due: LocalDateTime
    ): ReminderParseResult.Parsed {
        val title = original
            .replace(pattern, " ")
            .replace(DAY_WORDS, " ")
    ...
    private companion object {
        val RELATIVE_MINUTES = Regex("(?i)\\b(?:after\\s+|in\\s+)?(\\d{1,5})\\s*(?:minute|minutes|min|mins|मिनट)(?:\\s+(?:mein|me|में|ke\\s+baad|के\\s+बाद|baad|बाद))?\\b")
        val RELATIVE_HOURS = Regex("(?i)\\b(?:after\\s+|in\\s+)?(\\d{1,3})\\s*(?:hour|hours|hr|hrs|ghante|घंटे)(?:\\s+(?:mein|me|में|ke\\s+baad|के\\s+बाद|baad|बाद))?\\b")
        ...
        val TIME_FILLERS = Regex("(?i)\\b(at|after|baad|ko|में|me|mein|baje|बजे|morning|subah|सुबह|afternoon|dopahar|दोपहर|evening|shaam|शाम|night|raat|रात|ka|ki|ke)\\b")
    }
}
...
class MayraReminderStore(context: Context, private val maxEntries: Int = 500) {
    ...
    @Synchronized
    fun all(): List<MayraReminder> = runCatching {
    ...
    fun active(): List<MayraReminder> = all()
    ...
    fun markNotified(
        ...
        if (it.revision != expectedRevision || it.dueAt != expectedDueAt) return@update null
        it.copy(
            state = ReminderState.DUE,
            updatedAt = now.coerceAtLeast(it.createdAt),
    ...
    private fun MayraReminder.nextRevision(
        state: ReminderState,
        now: Long,
    ...
    private fun write(items: List<MayraReminder>) {
        ...
        check(preferences.edit().putString(KEY_ITEMS, array.toString()).commit()) {
            "Unable to persist reminders."
        }
    }
    ...
    private fun MayraReminder.toJson() = JSONObject()
        ...
        .put("state", state.name).put("followUp", followUpEnabled)
        .put("notificationCount", notificationCount).put("lastNotifiedAt", lastNotifiedAt)
        .put("revision", revision)

    private fun JSONObject.toReminder() = MayraReminder(
        id = getString("id"),
        title = getString("title").take(160),
        detail = optString("detail").takeIf(String::isNotBlank)?.take(500),
        dueAt = getLong("dueAt"),
        createdAt = getLong("createdAt"),
        updatedAt = getLong("updatedAt"),
        state = runCatching { ReminderState.valueOf(getString("state")) }.getOrDefault(ReminderState.SCHEDULED),
        followUpEnabled = optBoolean("followUp", true),
        notificationCount = optInt("notificationCount", 0).coerceAtLeast(0),
        lastNotifiedAt = if (isNull("lastNotifiedAt")) null else optLong("lastNotifiedAt"),
        revision = optLong("revision", 0L).coerceAtLeast(0L)
    )

    private companion object {
        const val PREFS = "mayra_owned_reminders_v1"
